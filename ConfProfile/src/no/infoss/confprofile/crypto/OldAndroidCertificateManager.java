 ...
 *
 * Original file was obtained from strongSwan project source code.
 * [Modifications] Copyright (C) 2014 Dmitry Vorobiev
 *
...
/*package*/ class OldAndroidCertificateManager {
	...
	private void loadCertificates() {
		...
		try {
			...
			this.mFingerMap.clear();
			for(Entry<String, X509Certificate> entry : this.mCACerts.entrySet()) {
				String fingerprint = CryptoUtils.formatFingerprint(entry.getValue().getSignature());
				this.mFingerMap.put(fingerprint, entry.getKey());
			}
			this.mLoaded = true;
	...
	public X509Certificate getCACertificateFromFingerprint(String fingerprint) {
		this.mLock.readLock().lock();
		String alias = this.mFingerMap.get(fingerprint);
		this.mLock.readLock().unlock();
		
		return getCACertificateFromAlias(alias);
	}
