...
public class IpSecVpnStateService extends Service
{
	...
	{
		public IpSecVpnStateService getService()
		{
			return IpSecVpnStateService.this;
		}
	}
	...
	public void startConnection(final Map<String, Object> options)
	{
		notifyListeners(new Callable<Boolean>() {
			...
			{
				IpSecVpnStateService.this.mConnectionID++;
				IpSecVpnStateService.this.mState = State.CONNECTING;
				IpSecVpnStateService.this.mError = ErrorState.NO_ERROR;
				IpSecVpnStateService.this.mImcState = ImcState.UNKNOWN;
				IpSecVpnStateService.this.mRemediationInstructions.clear();
				return true;
			}
		});
	}
	...
	public void setState(final State state)
	{
		notifyListeners(new Callable<Boolean>() {
			...
			public Boolean call() throws Exception
			{
				if (IpSecVpnStateService.this.mState != state)
				{
					IpSecVpnStateService.this.mState = state;
					return true;
				}
				return false;
			}
	...
	public void setError(final ErrorState error)
	{
		notifyListeners(new Callable<Boolean>() {
			...
			public Boolean call() throws Exception
			{
				if (IpSecVpnStateService.this.mError != error)
				{
					IpSecVpnStateService.this.mError = error;
					return true;
				}
				return false;
			}
	...
	public void setImcState(final ImcState state)
	{
		notifyListeners(new Callable<Boolean>() {
			...
			public Boolean call() throws Exception
			{
				...
				if (IpSecVpnStateService.this.mImcState != state)
				{
					IpSecVpnStateService.this.mImcState = state;
					return true;
				}
				return false;
			}
	...
	public void addRemediationInstruction(final RemediationInstruction instruction)
	{
		mHandler.post(new Runnable() {
			...
			public void run()
			{
				IpSecVpnStateService.this.mRemediationInstructions.add(instruction);
			}
		});
	}
}
